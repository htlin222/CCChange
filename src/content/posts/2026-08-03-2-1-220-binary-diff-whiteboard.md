...
沒有新版。而且這次的「沒有」比昨天更值得講。

檢核時間是 2026-08-03T00:10:55Z（台北時間 08:10）：
...
今天沒有 24 小時內的新版，這是連續第九天。

| 這篇挖到的東西 | 判決 | 你該做什麼 |
| --- | --- | --- |
| 217 小時沉默，史上第二長 | 值得留意 | 什麼都不用做。但如果你有巡檢腳本，可以開始準備接一個比較大的版本 |
| 2.1.220 沒有任何新的使用者可見字串 | 確認 | 升不升都行，介面不會有一個字不一樣 |
| npm 主套件只是 22 KB 的殼 | 做鏡像的人必讀 | 私有 registry 要鏡像九個套件，不是一個 |
| `/whiteboard` 存在但被關著 | 好玩，今天沒用 | 不要去試，`isEnabled` 寫死 false |
| 系統提示詞的四處改動 | 有實質影響 | 沒有動作，但可以解釋為什麼最近感覺它比較不愛說教 |
| workspace trust 是整個 repo 粒度 | 安全相關 | 掛別人的 repo 前想一下，信任會擴及它所有 worktree |
